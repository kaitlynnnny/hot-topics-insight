...
7. [最终报告如何生成](#7-最终报告如何生成)
8. [文件目录与职责说明](#8-文件目录与职责说明)
9. [如何运行本项目](#9-如何运行本项目)
10. [如何修改和扩展](#10-如何修改和扩展)
...
- **信息过载**：每天成千上万条新闻，你不知道该关注什么
- **单一视角**：只看一家媒体或一个 AI，容易被带偏
- **真假难辨**：社交媒体上假新闻泛滥，需要一个自动化的验证机制
- **时间有限**：你不可能每天手动读完所有新闻再做分析

---

# 2. 核心概念解释

在阅读后续内容之前，需要先理解几个关键概念。

## 2.1 什么是 LLM（大语言模型）？

LLM（Large Language Model，大语言模型）是一种人工智能程序，它能理解和生成人类语言。你问它一个问题，它给你一个回答。

本项目中使用了三个 LLM：

| 名称 | 开发公司 | 类比 |
|------|---------|------|
| **DeepSeek** | 中国深度求索公司 | 一个很聪明但知识截止到 2024 年的分析师 |
| **Qwen（通义千问）** | 中国阿里巴巴 | 另一个聪明但同样知识有截止日期的分析师 |
| **Gemini** | 美国 Google | Google 的最新模型，知识较新 |

这些模型本身不知道 2026 年 7 月正在发生什么——它们的训练数据有截止日期。这就是为什么我们需要**联网搜索验证**来帮它们理解当前正在发生的事。

## 2.2 什么是 API Key？

API Key 是一串密码，用来向 AI 公司证明"我有权限使用你们的模型"。就像酒店房卡——没有它你进不了房间。

每个模型需要各自的 API Key，配置在 `.env` 文件中：
```
DEEPSEEK_API_KEY=sk-xxxxxxxx    # DeepSeek 的钥匙
QWEN_API_KEY=sk-xxxxxxxx        # Qwen 的钥匙
GEMINI_API_KEY=AIza-xxxxxxxx    # Gemini 的钥匙
```

## 2.3 什么是 RSS？

RSS（Really Simple Syndication）是网站提供的一种标准格式，专门用来让别人订阅和抓取内容。几乎所有新闻网站都有 RSS 输出。

```
新闻网站（如 BBC） → 自动生成 RSS 文件 → 我们的程序定期读取 → 获取最新新闻列表
```

RSS 就像新闻网站的"自动播报机"——网站更新了新闻，RSS 文件自动更新，我们不间断地获取最新内容。

## 2.4 什么是 Embedding？

Embedding（向量嵌入）是把一段文字转换成一串数字（向量）的技术。这段数字代表了文字的"含义"。两段意思相近的文字，它们的向量在数学空间里也相近。

```
"曼谷酒吧火灾 27 人死亡" → [0.23, -0.45, 0.78, ...] (384 个数字)
"Deadly fire at Bangkok pub kills 27" → [0.21, -0.43, 0.76, ...] (384 个数字)

这两组数字非常接近！→ 系统判定：同一事件
```

## 2.5 什么是 JSON 和 JSONL？

- **JSON**: 一种结构化的数据格式，用来在程序之间传递信息。看起来像这样：
  ```json
  {"title": "曼谷大火", "source": "BBC", "summary": "..."}
  ```

- **JSONL**: "JSON Lines" 的缩写。每一行是一个独立的 JSON 对象。本项目的数据存储格式就是 JSONL。
...
在本项目中，Glean 安装在 `C:\Users\21947\glean\` 目录下。
...
每个新闻源是一个独立的"Feed"（信息流）。配置文件在 `C:\Users\21947\glean\feeds.yaml`。
...
`output/render.py` 将辩论结果和原始新闻列表渲染为一个自包含的 HTML 网页文件。

### 设计原则

- **白色/浅灰背景**：专业商务风格，适合打印和正式场合
- **不使用深色主题**：深色是 AI 聊天工具的典型风格，本项目刻意避免
- **Segoe UI 字体**：Windows 系统默认，专业、清晰
- **蓝色强调色**：传统企业报告配色
- **响应式布局**：在手机和平板上也能看
...
报告顶部的三个导出按钮通过浏览器端的 JavaScript 实现：

- **Export Excel (.csv)**：将辩论结果和新闻列表拼接为 CSV 字符串，添加 UTF-8 BOM（确保中文在 Excel 中不乱码），触发浏览器下载
- **Export Word (.doc)**：将数据拼接为 HTML 文档，使用 `application/msword` MIME 类型触发下载。Word 能直接打开 HTML 文件
- **Print / Save PDF**：调用浏览器的 `window.print()`，在打印对话框中选择"另存为 PDF"
...
所有路径相对于 `C:\Users\21947\hot-topics-insight\`
...
5. 参数: `-File "C:\Users\21947\hot-topics-insight\daily.ps1"`
...
编辑 `C:\Users\21947\glean\feeds.yaml`，在 `feeds:` 列表末尾添加：
...
然后在 `C:\Users\21947\glean\run.ps1` 的 `$feeds` 列表中加入 `src-新源名`。
...
**解决**: 设置环境变量：
```powershell
$env:GLEAN_DB_ROOT = "C:\Users\21947\glean"
$env:GLEAN_FILE_SINK_ROOTS = "C:\Users\21947\glean\output"
```
